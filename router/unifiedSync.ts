...
/**
 * Unified sync coordinator - runs all syncs in sequence
 * Order: Teams → Contacts → Matter Types → Statuses → Matters → Matter Details
 */
export const unifiedSync = authorized
  .route({
    method: "POST",
    path: "/sync/unified",
    summary: "Run complete data synchronization in correct order",
    tags: ["Sync"],
  })
  .output(unifiedSyncResultSchema)
  .handler(async ({ context }) => {
    const startTime = Date.now();
    const phases: z.infer<typeof syncPhaseResultSchema>[] = [];

    try {
      console.log("[UNIFIED-SYNC] Starting complete synchronization...");
      ...
      try {
        const refResult = await syncReferenceData(context.user.id);
        phases.push({
      ...
      try {
        const mattersResult = await syncMatters(context.user.id);
        phases.push({
          phase: "matters",
      ...
      try {
        const detailsResult = await syncMatterDetails(context.user.id);
        phases.push({
      ...
      return {
        ...
        message: allSucceeded
          ? "All synchronization phases completed successfully"
